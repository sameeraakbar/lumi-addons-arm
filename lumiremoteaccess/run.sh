...
if [ ! -d "$KEY_PATH" ]; then
  ...
  mkdir -p "$KEY_PATH"
  ssh-keygen -b 4096 -t ed25519 -N "" -C "hassio-setup-via-lumiremoteaccess" -f "${KEY_PATH}/autossh_rsa_key"
...
if [[ "${HTTP_STATUS_CODE}" -ne 200 && "${HTTPS_STATUS_CODE}" -ne 200 ]] ; then
  bashio::log.error "Testing HA socket '${FORWARD_LOCAL_SOCKET}' on the local system..."\
  ...
  bashio::log.info "Testing HA socket '${FORWARD_LOCAL_SOCKET}' on the local system... Web frontend reachable over HTTP"
elif [[ "${HTTPS_STATUS_CODE}" -eq 200 ]]; then
  bashio::log.info "Testing HA socket '${FORWARD_LOCAL_SOCKET}' on the local system... Web frontend reachable over HTTPS"
fi
